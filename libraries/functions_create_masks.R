...
#' coniferous-only, coniferous-flex and full composition.
#'
...
apply_and_save_masks <- function(raster,
  ...
  if (!dir.exists(file.path(metrics_dir, "Coniferous_Flex"))) {
    dir.create(file.path(metrics_dir, "Coniferous_Flex"), 
               showWarnings = FALSE, 
               recursive = TRUE)
  }
  if (!dir.exists(file.path(metrics_dir, "Coniferous_Only"))) {
    dir.create(file.path(metrics_dir, "Coniferous_Only"), 
               showWarnings = FALSE, 
               recursive = TRUE)
  }
